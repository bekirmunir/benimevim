...
mongoose.connect('mongodb://bmg_mongo:bmg123@localhost/bekon',{auth:{authdb:"bekon"}});
var mesajdata = {
    SERVER_MESSAGE:"Sistem otomatik açılış yapıyor..",
    UzakIp:'0.0.0.0',
    LocalIP:'0.0.0.0',
    UzakPort:'2222',
    YerelPort:'2222',
...
console.log(new Date(), 'Sulama gorev daemon 8 sn içinde başlayacak.İnşALLAH');
    ...
    Client.on('connection',function(socket){
        ...
        socket.on('ButtonSet', function(butdata) {
            // cihaznet.login(io, socket, data);
            if(butdata.button.indexOf('Lamba') > -1) {
        ...
        socket.on('ElkDegerAl', function(data) {
            // cihaznet.login(io, socket, data);
            console.log("'ElkDegerAl  data:"+data);
...
function SulamaGorevOkuFn(Clin) {
    console.log('burası sulama gorevoku fonksiyonu');
